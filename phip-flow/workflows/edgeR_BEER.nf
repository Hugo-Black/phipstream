// edgeR fits the beads-only background and BEER scores enrichment from the
// resulting PhIPData object.


// Export the phippery dataset to wide CSV tables.
process to_csv {
...
process run_BEER {
    ...
    shell:
    """
    run_BEER.Rscript ${params.beer_min_lib_size} \
        ${params.beer_posterior_threshold} \
        ${params.beer_beads_rr ? 'TRUE' : 'FALSE'} \
        ${params.beer_seed}
    """
...
// Add scoring CSV outputs back into the xarray dataset.
